...
describe('62', function() {
  ...
  test('Arrayn innehåller 3 textsträngar', () => {
    const assignments = getVariableAssignments(data.ast, true);
    if(!assignments || assignments.size < 1)
      throw new Error('Du måste tilldela ett värde till din variabel.');
    if(assignments.size > 1)
    ...
    if(arr.elements.length !== 3)
      throw new Error('Arrayn måste innehålla 3 element.');
    for(let i = 0; i < arr.elements.length; i++) {
      if(typeof nodeToLiteralString(arr.elements[i]) !== 'string')
        throw new Error('Alla element i arrayn måste vara textsträngar.');
    }
  });
});
