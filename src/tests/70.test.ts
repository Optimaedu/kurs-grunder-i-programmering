...
describe('70', function() {
  ...
  test('Ändra inte på array deklarationen', () => {
    ...
    const declarations = getVariableDeclarations(data.ast).get('numbers');
    if(!declarations || declarations.length < 1)
      throw new Error(errMessage);
    if(declarations.length > 1)
      throw new Error('Du får endast deklarera en `numbers` variabel.');
    ...
    for(let i = 0; i < correct.length; i++) {
      if(nodeToLiteralNumber(elements[i]).toString() !== correct.charAt(i))
        throw new Error(errMessage);
    }
  });

  test('Använder array funktionen `sort()`', () => {
    const calls = getFunctionCalls(data.ast).get('numbers.sort');
    if(!calls || calls.length < 1)
      throw new Error('Du måste använda array funktionen `sort()`\nför att sortera arrayn.');
  });
  ...
  test('Använder arrayn funktionen `join()` i `console.log()`', () => {
    ...
    if(argCall.callee.type !== 'MemberExpression')
      throw new Error('Du skall använda array funktoinen `join()` när\ndu skriver ut arrayn i `console.log()`.');
    if(nodeToIdentifierName(argCall.callee.object) !== 'numbers')
      throw new Error('Du skall använda array funktoinen `join()` när\ndu skriver ut arrayn i `console.log()`.');
